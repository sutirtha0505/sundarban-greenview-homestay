...
export default function HeroPage() {
  ...
  useEffect(() => {
    fetchLiveHeroData().then((data) => {
      setHeroData(data);
    });
  }, []);

  const roomImages = heroData.images && heroData.images.length > 0 ? heroData.images : defaultHeroData.images;
    ...
    <div id="home" className="relative h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 flex h-full w-full flex-col justify-top px-4 sm:px-8 lg:px-16 pt-35 sm:pt-32">
        <div className="mb-6 w-full">
          <h1
            className={`mb-4 sm:mb-6 text-4xl sm:text-7xl lg:text-[5.5rem] text-white leading-tight tracking-tight ${gloock.className}`}
        ...
        <div className="mt-4 sm:mt-6 flex w-full flex-col items-start gap-8 md:flex-row md:items-end">
          <Link
            href={heroData.buttonLink || "/booking"}
            className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-[#6DA003] px-8 py-3.5 text-lg text-white transition-all hover:bg-[#5B8703] border border-[#8FCE05] shadow-[0_10px_24px_rgba(109,160,3,0.24)]"
          >
            <span className="relative z-10 font-medium">{heroData.buttonText || "Book your Trip"}</span>
            <svg
            ...
            >
              {roomImages.map((src, index) => (
                <button
                  ...
                  className={`relative h-28 w-36 sm:h-32 sm:w-48 shrink-0 overflow-hidden rounded-xl group transition-all duration-300 snap-center ${
                    index === currentImageIndex ? "border-[3px] border-[#C5FE4E]" : "border-[3px] border-transparent"
                  }`}
                >
                  <CachedImage
                    src={src}
            ...
            {roomImages.map((_, index) => (
              <button
                ...
                className={`rounded-full transition-all duration-300 ${
                  index === currentImageIndex ? 'w-4 h-2 bg-white' : 'w-2 h-2 bg-white/50'
                }`}
